...
3. **Afficher le plan**, puis **démarrer directement** — **jamais** de confirmation `ok` / `go` / « je continue ? ». **Seule exception** (rare, dernier recours) : demande **inactionnable** → **une** question courte puis enchaîner. Doute non bloquant → choisir l'option la plus raisonnable, l'indiquer en une ligne, continuer.
...
7. **Preuve visuelle mobile** : pour toute tâche touchant l'**UI mobile** (Expo / React Native), déléguer à l'agent **mobile-preview** pour **booter un simulateur/émulateur** et **capturer un screenshot** (preuve affichée dans le fil, jamais commitée). Best-effort — ne bloque pas si aucun simulateur n'est disponible ; ignorer pour toute tâche non visuelle.
8. **Passe qualité finale** (automatique, **sans** confirmation) avant de retirer le draft de la PR : revue **cinq axes** (`code-review-and-quality`) + agent **verifier** (tests / lint / build relancés), et preuve visuelle mobile pour les écrans modifiés.
9. **Fin de cycle** : si une issue Linear est la source, l'agent **linear** propose de passer le ticket à *In Review* / *Done* (passage en état terminal **uniquement** sur accord explicite).
...
- **mobile-preview** — dev mobile : boot simulateur/émulateur + screenshot comme preuve (jamais commitée)
