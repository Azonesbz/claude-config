...
# The flow-pipeline skill mandates targeted `git add <paths>` and forbids `git add -A`,
...
if printf '%s' "${command_str}" | grep -Eq "${WHOLE_TREE_ADD}"; then
  hook_deny "Blocked by flow rules: stage only this task's files with git add <path>, never git add -A / --all / . (see the flow-pipeline skill)."
fi
exit 0
